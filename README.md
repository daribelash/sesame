...
checking a code before you arrive, without needing geocoding or a maps API.
...
**No ORM, no map SDK.** Raw SQL via `pg`, and a hand-written ~30-line haversine implementation
for distance — there's no API key or billing surface anywhere in the project.
...
# frontend, proxying /api to the backend below
npm run dev
...
No maps, geocoding, navigation, or routing. No sharing codes between users, and no crowdsourced
